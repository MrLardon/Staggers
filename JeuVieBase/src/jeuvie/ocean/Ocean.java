...
public abstract class Ocean implements IOcean {
	...
	public void etapeSuivante(){
		//TODO 2 etapes 1) super.clone 2) les composants TODO
	
	}
	public int compterVoisinsVivants(int iCol, int jLg) {
        int compteur=0;
        int bordGauche = iCol-1;
        int bordDroit = iCol+1;
        int bordHaut = jLg-1;
        int bordBas = jLg+1;
        int indiceDerniereColonne = this.getNbColonnes()-1;
        int indiceDerniereLigne = this.getNbLignes()-1;

        if (bordGauche<0) {
            bordGauche=0;
        }

        if (bordDroit > indiceDerniereColonne ) {
            bordDroit = indiceDerniereColonne;
        }
        ...
        if (bordBas > indiceDerniereLigne) {
            bordBas = indiceDerniereLigne;
        }

        for (int j = bordHaut; j <= bordBas; j++) {
            ILigneEau ligne = this.get(j);
            for (int i = bordGauche; i <= bordDroit; i++) {
                ICase uneCase = ligne.get(i);
                if (uneCase.contientBestioleVivante()) {
                    if(j!=jLg && i!=iCol) {
                        compteur ++;
                    }
                }
